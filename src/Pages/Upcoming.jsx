...
const Upcoming = () => {
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/movie/upcoming?api_key=711b673e8a9d9a73798bfbd7f7e018b7&language=en-US&page=${popularPage}`
    )
      .then((res) => res.json())
  ...
  return (
    <>
      ...
      {!popularArray ? (
        ...
        <div
          className="px-5 md:px-10 mb-28 flex flex-col md:flex-row gap-10 justify-between"
          onClick={() => {
            if (subActions > 0) {
              setSubActions(0);
            } else return null;
              ...
              {popularArray &&
                popularArray
                  ...
                  .map((result) => {
                    return (
                      <div
                        className="transition-all duration-700 md:w-1/4 w-[45%] border pb-1 rounded-lg bg-white md:hover:scale-105"
                        key={result.id}
                      >
